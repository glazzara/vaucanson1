...
namespace vcsn {
  ...
  do_backward_closure_here(const AutomataBase<A_>&, Auto& a)
  {
    ...
    typedef std::vector<std::vector<series_set_elt_t> > matrix_series_set_elt_t;
    ...
    matrix_semiring_elt_final_t	m_wfinal(size, init),
			        m_wfinal_tmp(size, init);

    for (i = 0; i < size; i++)
      {
	m_semiring_elt[i].resize(size, semiring_elt_zero);
	m_semiring_elt_tmp[i].resize(size,semiring_elt_zero);
	m_series[i].resize(size, series_set_elt_t(a.series()));
	m_series_ret[i].resize(size, series_set_elt_t(a.series()));
      }
	...
	a.set_final(index_to_state[i],
		    series_set_elt_t(a.series(), m_wfinal[i]));
    }
  }
