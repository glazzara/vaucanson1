// shuffle.hxx: this file is part of the Vaucanson project.
//
...
//
#ifndef VCSN_ALGORITHMS_SHUFFLE_HXX
# define VCSN_ALGORITHMS_SHUFFLE_HXX
...
class Shuffle
{
  public:
    ...
    Shuffle (const structure_t& structure,
    ...
    // returns the shuffle of @c lhs and @c rhs (and put it also in @c output)
    output_t&
...
# undef eq_
    ...
    inline bool
    is_shuffle_not_null (const lhs_t& lhs,
			 const rhs_t& rhs,
	 ...
	 const bool use_geometry)
{
  ...
  Shuffle<A, T, U> do_shuffle(ret.structure(), use_geometry);
  return do_shuffle (ret, lhs, rhs, m);
}
	 ...
	 const bool use_geometry)
{
  std::map<typename T::hstate_t,
    std::pair<typename T::hstate_t, typename U::hstate_t> > m;
  return shuffle (lhs, rhs, m, use_geometry);
}

} // End of namespace vcsn.

#endif // ! VCSN_ALGORITHMS_SHUFFLE_HXX
