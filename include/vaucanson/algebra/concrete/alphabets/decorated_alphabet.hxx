...
#ifndef VCSN_ALGEBRA_DECORATED_ALPHABET_HXX
# define VCSN_ALGEBRA_DECORATED_ALPHABET_HXX
...
namespace vcsn {
  ...
  template <typename L, typename T>
  L MetaElement<AlphabetSet<L>, AlphabetDecorator<L, T> >::joker() const
  {
    return value().joker();
  }

  template <typename L, typename T>
  L MetaElement<AlphabetSet<L>, AlphabetDecorator<L, T> >::other() const
  {
    return value().other();
  }
  ...
  { 
    ...
    if (v == a.other())
      return false;
    return Element<AlphabetSet<L>, T>(s, a.alphabet()).contains(v); 
  }
  ...
  bool op_letter_equality_modulo(const AlphabetSet<L>& s, 
				 ...
				 L rhs)
  {
    Element<AlphabetSet<L>, AlphabetDecorator<L, T> > e(s, a);
    if (lhs == a.joker()) 
...
#endif // VCSN_ALGEBRA_DECORATED_ALPHABET_HXX
