...
#define AUTOMATON_TYPES_EXACT_(AutoType,Prefix)				\
  ...
  typedef AutoType::transitions_t				Prefix##transitions_t; \
  typedef AutoType::transition_iterator			Prefix##transition_iterator; \
...
#define for_each_transition(E, A)			\
  for (transition_iterator E = A.transitions().begin();	\
       E != A.transitions().end();			\
       ++E)
