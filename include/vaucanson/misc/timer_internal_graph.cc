...
namespace misc
{
  namespace timer
  {
    ...
    INLINE_TIMER_CC
    void
    GraphCall::add_times (TimeStamp&	_total,
			  TimeStamp&	_self,
			  TimeStamp&	_program,
			  unsigned int	cnt)
    {
      ...
      total.wall      += _total.wall_;
      total.user      += _total.user_;
      total.system    += _total.sys_;
      total.cpu       += _total.sys_ + _total.user_;
      total.average   = total.cpu / double (count);
      total.charge    = total.cpu * 100 /
	                double (_program.sys_ + _program.user_);
      self.wall       += _self.wall_;
      self.user       += _self.user_;
      self.system     += _self.sys_;
      self.cpu        += _self.sys_ + _self.user_;
      self.average    = self.cpu / double (count);
      self.charge     = self.cpu * 100 /
	                double (_program.sys_ + _program.user_);
    }

    INLINE_TIMER_CC
    void
    GraphCall::add_self_time (TimeStamp&		_self,
			      unsigned int		cnt)
    {
      count         += cnt;
    ...
    INLINE_TIMER_CC
    void
    GraphCall::add_total_time (TimeStamp&		_total,
			       unsigned int		cnt)
    {
      count         += cnt;
    ...
    INLINE_TIMER_CC
    void
    GraphTask::add_times (TimeStamp&	_total,
			  TimeStamp&	_self,
			  TimeStamp&	_program,
			  unsigned int	cnt)
    {
    ...
    INLINE_TIMER_CC
    void
    GraphTask::add_int_time (TimeStamp&		_self,
			     unsigned int	cnt)
    {
      int_count    += cnt;
