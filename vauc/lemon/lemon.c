...
void ErrorMsg(int, ...  /* char *, int, char *, ... */ );
...
struct lemon {
  ...
  int  outlocal;           /* Output to local directory */
...
struct lemon *lemp;
{
  ...
  ** start nonterminal.  */
  ...
  for(rp=lemp->rule; rp; rp=rp->next){
    ...
    ErrorMsg(0, lemp->filename,rp->ruleline,"This rule can not be reduced.\n");
    lemp->errorcnt++;
  }
}
  ...
  for(cfp=current; cfp; cfp=cfp->next){
    ...
    if( sp->type==NONTERMINAL ){
      if( sp->rule==0 && sp!=lemp->errsym ){
        ErrorMsg(0, lemp->filename,rp->line,"Nonterminal \"%s\" has no rules.",
          sp->name);
        lemp->errorcnt++;
      }
...
char **argv;
{
  ...
  static struct s_options options[] = {
    ...
    {OPT_FLAG, "l", (char*)&outlocal, "Output parser to current directory."},
    {OPT_FLAG,0,0,0}
  };
  int i;
  struct lemon lem;
  ...
  switch( psp->state ){
    ...
    case WAITING_FOR_DECL_OR_RULE:
      if( x[0]=='%' ){
        ...
        if( psp->prevrule==0 ){
          ErrorMsg(0, psp->filename,psp->tokenlineno,
...
struct lemon *gp;
{
  ...
  if( fp==0 ){
    ErrorMsg(0, ps.filename,0,"Can't open this file for reading.");
    gp->errorcnt++;
    return;
  }
  fseek(fp,0,2);
  ...
  if( filebuf==0 ){
    ErrorMsg(0, ps.filename,0,"Can't allocate %d of memory to hold this file.",
      filesize+1);
    gp->errorcnt++;
    return;
  }
  if( fread(filebuf,1,filesize,fp)!=filesize ){
    ErrorMsg(0, ps.filename,0,"Can't read in all %d bytes of this file.",
      filesize);
    free(filebuf);
    gp->errorcnt++;
    return;
  ...
  for(cp=filebuf; (c= *cp)!=0; ){
    ...
    if( c=='\"' ){                     /* String literals */
      ...
      if( c==0 ){
        ErrorMsg(0, ps.filename,startline,
"String starting on this line is not terminated before the end of the file.");
      ...
      if( c==0 ){
        ErrorMsg(0, ps.filename,ps.tokenlineno,
...
char *suffix;
{
  ...
  if ( lemp->outlocal && (cp = strrchr(lemp->filename, '/')) )
    strcpy(name,cp + 1);
  else
...
int *lineno;
{
 ...
 if( rp->code ){
   ...
   for(cp=rp->code; *cp; cp++){
     if( isalpha((unsigned char)*cp) && (cp==rp->code || !isalnum((unsigned char)cp[-1])) ){
       char saved;
       for(xp= &cp[1]; isalnum((unsigned char)*xp); xp++);
       saved = *xp;
       *xp = 0;
 ...
 if( rp->lhsalias && !lhsused ){
   ErrorMsg(0, lemp->filename,rp->ruleline,
