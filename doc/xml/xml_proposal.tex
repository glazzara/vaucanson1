...
community a communication tool that could be used for the connection
...
As it will be described further, we use a XSD file (see \cite{XSD})
for the description format of this XML proposition, since it allows
context sensitive declarations.  This report first presents a XML
...
distinguished in this proposition. They are described in the two
following tags.  The \geometrytag{} data correspond to the embedding
...
The proposed policy supposes these properties to be checked by the
...
automaton. It has two children which are mandatory and supposed to
appear in a specific order. These tags enable definitions of states
and transitions.
...
\srcname{} and \dstname{}, representing source and destination of the
...
In order to avoid some potential problems, the user can set the value
of the empty word with the attribute \textit{identity\_symbol} of the
monoid. This way, the empty word symbol can always be different from
any character of the used alphabet.
...
\begin{itemize}
\item \texttt{type}\\
  This is used to set the type of the monoid. Choices are unit, free
  or product.
...
\item \texttt{range}\\
  ...
  the symbols one by one. For instance, the range ascii sets the
  alphabet on the ascii characters.
\end{itemize}
...
elements are mandatory ones.  The tags \geometrytag{} and
\drawingtag{} can be placed at any place of the document and are quite
special. They are ignored here, but more information can be found in
...
represented by ``1'', and the absorbent element of the semiring by
``0''.  These are default values in \Vauc.
...
default type for transducers.  The \texttt{operations} attributes is
...
two attributes can be set. Firstly, the \texttt{transitionType}
...
transition as a left arc.  {\small
...
The most important difference with our previous proposal (see
...
This is not possible with a DTD description.  The use of a XSD
