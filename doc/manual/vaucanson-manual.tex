...
that should be called from the shell and that can be chain operations
on automata.

\section{\tafkit Instances}

Because of the generic programming paradigm used in the \Vauc library,
the types of the automata manipulated needs to be known at
compile-time.  \tafkit has therefore been compiled for several
predefined types of automata.  It is actually the same program, but
...
\command{vcsn-char-z} associate each word to an integer.

\Vauc also supports transducers, that are able to read and write
weighted words.  These transducers are actually automata over a
product two free monoids.  In \Vauc we call these FMP, for \emph{free
  monoïd products}.  The above table lists a few \tafkit instance for
FMP.

\section{A First Contact}\label{sec:firstcontact}

If you have fully installed \Vauc (\autoref{sec:building}) you should
be able to just type any of the following commands and observe their
results.  If you only compiled \Vauc without installing it, you should
\command{cd} into the \file{vaucanson-\VcsnVersion/taf-kit/tests/}
directory and type \kbd{./vcsn-char-b} instead of \kbd{vcsn-char-b}
...
When \tafkit reads an XML file, there is not need to specify any other
...
expression, \Vauc need to know what alphabet it its using.  Here there
...
When specifying characters alphabets, some characters like \samp{ } (space),
\samp{"}, \samp{(}, \samp{)}, and \samp{'} have to be escaped.  For
instance the following command will create an automaton that
recognize numbers of the form \samp{12,456,789}, where a comma must
be used as thousand separator: note how the comma must be escaped in the
alphabet
...
\paragraph{Verbose Boolean results}
...
\subsection{Empty word and null series}
...
For integer alphabet, the empty word is of course always \samp{e}.

Similarly the symbol use to represent the null series defaults the
first representation from the following list that is not in conflict
with the alphabet: \samp{0}, \samp{z}, \samp{\_z}, \samp{zero}.

Next section shows how you can actually specify symbols of your own
for these tokens.


\subsection{Specifying writing data}\label{sec:writingdata}

Previous section showed how the empty word and null series can have
different symbols.  \Vauc actually allows other symbols used in
rational expression to be changed.
