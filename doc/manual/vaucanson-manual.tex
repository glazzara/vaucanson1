...
\tafkit is command-line interface to \Vauc.  It is a set of programs
...
\begin{shell}
$ \kbd{vcsn-char-b determinize a1.xml \textbackslash| info -}
States: 4
Transitions: 8
Initial states: 1
Final states: 2
...
\section{\tafkit's Modus Operandi}
...
The \emph{internal pipe}, \samp{\textbackslash|}, is used to separate
...
\begin{enumerate}
\item parse any options
\item parse all expected arguments
      (using indications that may have been given as option)
\item apply the algorithm
\item print the result (in a format that can be controlled using
  options)
\end{enumerate}

When commands are chained internally using \samp{\textbackslash|} and
...
Here we only cover the options that may modify the Input/Output
behaviors of commands.

\subsection{Specifying an alphabet}
...
\begin{shell}
$ \kbd{./vcsn-char-b standard aba+a}
Error: alphabet should be explicitly defined using --alphabet
\end{shell}%$

\command{standard} is a command that takes a rational expression and
...
\begin{shell}
$ \kbd{./vcsn-char-b standard --alphabet=ab aba+a > aut.xml}
\end{shell}%$
...
\begin{shell}
$ \kbd{./vcsn-char-b standard -aab aba+a > aut.xml}
\end{shell}%$
