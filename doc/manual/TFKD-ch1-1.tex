...
% %
%   \end{equation*}
  o~$m$ est n'importe quel ŽlŽment du mono•de.
  
  De toutes faons cette identitŽ doit tre revue ˆ la lumire de 
  l'Žcriture de  pour les weighted automata.
\end{ComVd}%
}%
...
% \{2\} (ab \{3\})
% $ \kbd{vcsn-char-z -aab cat-E '\{2\}a\{3\}'}
% \{6\} a
% \end{shell}%$
...
corresponds to what is actually implemented in \vcsnv and is somehow 
a mistake.
A more natural definition would be 
  $\msp m\{k\}\Rightarrow \{k\}m\msp$
with~$m$ any element of the monoid.
This may be corrected in forthcoming revisions of \vcsnv but should 
anyway be reevaluated in connection with the definition of the 
function \code{derived-term} for the weighthed automata.

% When we wrote `denoted' above, we refer to the way we represent 
% expressions \emph{in this document}.
% We describe now how the same expressions are \emph{read} (as in the 
% \FctInd{exp-to-aut}) or \emph{written} (as in the 
% \FctInd{aut-to-exp}) by \vcsn. 
...
A rational espression output by \Fct{cat-E} is in reduced form (\cf 
\sbsct{red-exp}).
\index{expression!reduced}%
...
the labels of initail and final arrows are restricted to be 
\emph{weights}.
