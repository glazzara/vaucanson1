...
The chapter is a work in progress.  It is not meant for users of the
\Vauc library, but for developers and those who wish to contribute
code to \Vauc.
...
We use a number of tools during development which we call
...
\begin{description}
\item[vaucanson@lrde.epita.fr] General discussions, feature requests etc.
...
\item not strongly typed (can be dumped from one automaton type and
  loaded to another).
\end{itemize}

Drawbacks relative to XML:

\begin{itemize}
\item not strongly typed (one cannot know what automaton type to
