...
% \item \Fctaut{reverse}
\item \Fctaut{accessible}, \Fctaut{coaccessible}%, \Fctaut{is-accessible} 
...
\begin{enumerate}
\item \Fctaut{proper}, \Fctaut{is-proper}
\item \Fctaut{standardize}, \Fctaut{is-standard}
...
\begin{enumerate}
\item \Fctaut{aut-to-exp}, \Fctaut{aut-to-exp-DM}, 
...
\begin{ComVd}{110724}
    ...
    les commandes en \code{is-}: \Fctaut{is-accessible}, 
    \Fctaut{is-coaccessible}.
    
    \thii Déplacées, ou posent problème:
    
    \Fctaut{support}, \Fctaut{characteristic};

    \Fctexp{expand}.
	
	\thiii Implémentée avec un scope restreint: \Fctexp{exp-to-aut}.
    
    \thiv Non documentées, cachées, mais existent quand même:
...
We follow the definition taken in~\cite{Saka03,Saka09} and consider 
that the result is defined if, and only if, the family of weights of 
...
It is (will be indeed) documented in~\cite{LombSakaXX}.
...
\longonly{%
\begin{ComVd}{110724}
J'ai refait les tests avec next-168 (du 14/07).

Je ne comprends pas très bien ce qui a été inclus comme algorithme
proper dans cette version, mais il donne un automate pour des valeurs
1 et 1/2 du poids de la transition 3, alors qu'il devrait sortir en
erreur.  Je suis allé voir \code{eps\_removal.hxx} , ça a pourtant l'air
d'être le programme écrit par Sylvain.  Mystère, mystère.
\end{ComVd}
}%
...
% 
% \begin{ComV}%{101205}
%     Bilan de la réunion du 2/12/10:
%     
%     Pas implémentée. 
% \end{ComV}
% 
...
%     
%     Pas implémentée. 
%     La spécification ci-dessus est contradictoire avec la règle 
%     supposée qu'une commande appartient à l'instance correspondant 
%     au type de l'argument d'entrée.
%     Pour satisfaire à cette convention, il faudrait que la commande 
%     admette un second argument en entrée qui indiquerait le 
%     semianneau des multiplicités.
%     
%     Décision reportée.
% \end{ComV}
% 
...
\Prec \Prm{a.xml} is \emph{standard}, for the left 
...
\longonly{%
\begin{ComVd}{110626}
	La dernière phrase renvoie à un premier test que j'ai fait sur 
	un exemple de matching d'expression rationnelle considéré par 
	Russ Cox avec son logiciel \code{RE2} (\cf 
	\code{http://swtch.com/$\sim$rsc/regexp/}) et par Thomas Wilke  
	dans son papier à ICFP'10 (\cf également 
	\code{http://sebfisch.github.com/haskell-regexp/}).
	
	Nous sommes évidemment encore très loin du compte mais ce n'est 
	pas l'objectif de \vcsn de faire du matching d'expressions 
	rationnelles.
\end{ComVd}
}
...
\subsubsection{\Fct{aut-to-exp}, \Fctaut{aut-to-exp-DM}, \Fctaut{aut-to-exp-SO}}
...
\begin{ComVd}{110724}
\thi 
Il ne serait pas inutile de vérifier que le \code{DMChooser} correspond 
bien aux spécifications de Delgado-Morais.

\thii 
Si on a accès à un générateur aléatoire d'automates, on pourrait 
reprendre sérieusement la comparaison entre les heuristiques.
...
is for weigted automata, as shown at \figur{III-2-24}, result of the 
...
\longonly{%
\begin{ComV}%{101205}
Il a été convenu que dans les versions ultérieures, la spécification 
de \Fct{exp-to-aut} consisterait à construire un automate standard 
dons les transitions sont étiquetées par les \emph{atomes} de 
l'expression. 
(La commande \Fct{standard} conservera sa spécification actuelle).
\end{ComV}
}
...
\longonly{%
\begin{ComVd}{110626}
	Fonction de service, pour présenter certains résultats de façon 
	plus naturelle et lisible.
	
	Dans mon souvenir, la distributivité s'effectuait récursivement 
	de gauche à droite, jusqu'à la première sous-expression étoilée, 
	et s'arrêtait alors, sans franchir cette sous-expression et sans 
	rentrer dedans.
	
	Je pense que cette fonction peut avoir différents comportements, 
	l'actuel, celui décrit précédemment, et peut-être d'autres, 
	commandés par un paramètre. Il faudrait aller y voir de plus près.
	
	J'ai l'impression que l'implémentation actuelle donne de chaque 
	expression une \emph{forme canonique} modulo les identités 
	$(\mathbf{T})$, $(\mathbf{A})$ (pour l'addition et le produit) et 
	$(\mathbf{C})$ (pour l'addition).
\end{ComVd}
}%
