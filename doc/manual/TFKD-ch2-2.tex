...
% 
\Apriori, there is no assumption on the multiplicity semiring.
...
\item \Fctaut{is-realtime}, \Fctaut{realtime}
\item \Fctaut{is-unambiguous}
\item \Fctaut{partial-identity}
...
\begin{enumerate}
\item \Fctaut{quotient}  %, \Fctaut{coquotient}
\item \FctautD{product}
\item \FctParD{power}{aut}{n}
\item \FctautD{shuffle}, \FctautD{infiltration}
\end{enumerate}
...
\begin{ComV}%{101205}
    \thi Pas implémentées:
%     
    \Fct{is-letterized}, \Fct{letterize}; \Fct{coquotient};
	\Fct{support};
%     \Fct{eval-S}; 
    \Fct{hadamard-S}, \Fct{shuffle-S}, \Fct{infiltration-S};

    les commandes en \code{co}: .
...
shown on the followng example (\cf \figur{c1} for the 
...
\subsubsection{\Fct{partial-identity}}
\label{ssc:par-ide}%
...
\longonly{%
\begin{ComVd}{110709}
	Another occurence of the usefulness of subliminal initial and 
	final states.
\end{ComVd}
}%
...
The function \Fct{aut-to-exp} and its variant (\cf 
...
expression which denotes a word.
\begin{shell}
...
\longonly{%
\begin{ComV}
 	{\Fct{enumerate}} et {\Fct{shortest}} déplacées pour les 
	automates booléens (\cf \sbsct{aut-boo-beh}).
\end{ComV}
}%


\subsection{From expressions to automata}
\label{ssc:aut-mul-eta}%
...
In the former case they are synonymous.
\end{ComVd}
}%
...
It is so in \tafkitv as it makes the reading of the parameter 
\Prm{alpha} easier.
\end{ComVd}
}%
...
\begin{ComVd}{110724}
	 On pourrait imaginer que \Fct{quotient} soit implémentée pour 
des automates non real-time, et même des transducteurs.
\end{ComVd}
}%
...
% 	\Fct{infiltration-S} ne sont pas implémentées dans \tafkitv.
% \end{ComVd}
% }
