...
\item \samp{>a1det.xml} puts the result of \taffn{determinize} into
  the file \file{a1.xml}, that is, the XML file which describes the
...
\execcaption{bz-mismatch}{vcsn-char-b dump-automaton a1 | vcsn-char-z info -}
...
use the \verb-ALPHABET- token an arbitrary number of time, to append letters
...
(\verb-a-, \verb-b- and \verb-c-), while \verb-ALPHABET=words:abc- defines the
...
%%
%% \execcaption{t1.110}{vcsn-char-fmp --alphabet1=ab eval t1.xml 'bba'}

\subsubsection{Domain}
The transducer $T$ only accepts binary numbers divisible by 3.

\execcaption{t1.dom}{vcsn-char-fmp dump-automaton t1 | vcsn-char-fmp --alphabet1=ab domain - >div-by-3.xml}
...
%% # vcsn-char-fmp to-rw-tdc add1.xml > add1_rw.xml
%% \end{shell}
%%
%% \subsubsection{Composing}
%% \begin{shell}
%% # vcsn-char-fmp compose quot_3.xml add1.xml
%% \end{shell}

\subsection{Available functions}
The following functions are available for both \command{vcsn-char-rw}
and \command{vcsn-char-fmp} programs.  To invoke them, run
\samp{\var{program} \var{algorithm-name} [\var{arguments}]}.

\execcaption{vcsn-char-fmp-commands}{vcsn-char-fmp --list-commands}
...
\command{vcsn-char-z}, see \autoref{sec:auto:z}.
