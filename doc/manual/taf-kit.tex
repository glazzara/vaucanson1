...
\index{t1@$\Tc_1$}$\Tc_1$ (\autoref{fig:t1}) gives the
quotient by 3 of a binary number and the transducer
\index{t2@$\Tc_2$}$\Tc_2$ (\autoref{fig:t2}) adds 1 to
a binary number.
...
automaton:
\execcaption{t1.xml}{vcsn-tdc dump-automaton t1 >t1.xml}
\execcaption{t1.dot}{vcsn-tdc dot-dump t1.xml >t1.dot}
\begin{center}
  \includegenerateddot{t1}
\end{center}


\subsubsection{Evaluation}

\execcaption{t1.110}{vcsn-tdc --alphabet1=01 eval t1.xml '110'}
...
\noindent
The file \file{b4.xml} now contains the automaton $\Bc_1^4$.
