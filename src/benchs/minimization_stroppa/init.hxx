...
namespace vcsn {
		    ...
		    const W&			word)
  {
    ...
    for_all_const_(W, letter_p, word)
    {
      ...
      a.add_letter_transition(current_state, new_state, (*letter_p));
		    ...
		    const W&			word)
  {
    ...
    for_all_const_(W, letter_p, word)
    {
      ...
      t.add_io_transition(current_state, new_state, (*letter_p), (*letter_p));
      current_state = new_state;
    }
...
//     b = minimization_moore(b);
    ...
    // Maps for states and transitions weights
    std::map<htransition_t, semiring_elt_value_t>     transitions_weights;
    ...
    for_each_transition(e_p, b) {
      transitions_weights[(*e_p)] = wzero;
    }
	...
	b.letter_deltac(cont, current_state, *l_p, delta_kind::transitions());
	assertion(cont.size());
	...
	final_weight = transitions_weights[(*transition_p)] / states_weights[origin];
      }

      series_set_elt_t s(b.series());
      s.assoc(b.word_of(*transition_p), final_weight);
      b.update(*transition_p, s.value()); // safe ?
    }
