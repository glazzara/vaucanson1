...
bool elimination_dmheuristic_test(tests::Tester& tg)
{
  ...
  for (unsigned nb = 0; nb < t.test_num(); ++nb)
	...
	SAVE_AUTOMATON_XML("/tmp", "aut_to_exp_DMheuristic", a, nb);
      unsigned i = 0;
      for (; i < nb_word_test; ++i)
      {
	monoid_elt_t w = language.choose_from_supp();
	if (t.verbose(tests::Tester::high))
	  std::cerr << "TEST: aut_to_exp_DMheuristic " << nb << " : test "
		    << w << std::endl;
	if (eval(a, w) ==
	    zero_as<semiring_elt_value_t>::of(a.structure().series().semiring()))
	{
	  break;
	  if (t.verbose(tests::Tester::high))
	    std::cerr << "TEST: aut_to_exp_DMheuristic " << nb
		      << " failed." << std::endl;
	}
      }
      if (i == nb_word_test)
  ...
  TEST(t, "aut_to_exp, Delgado-Morais heuristic, basic test " + rate,
       success == t.test_num());
  return t.all_passed();
}
