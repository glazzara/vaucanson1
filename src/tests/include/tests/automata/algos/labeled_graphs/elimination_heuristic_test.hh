// elimination_heuristic_test.hh: this file is part of the Vaucanson project.
// 
...
//
#ifndef VCSN_TESTS_AUTOMATA_ALGOS_LABELED_GRAPHS_ELIMINATION_HEURISTIC_TEST_HH
# define VCSN_TESTS_AUTOMATA_ALGOS_LABELED_GRAPHS_ELIMINATION_HEURISTIC_TEST_HH
...
template <class Auto>
bool elimination_heuristic_test(tests::Tester& tg)
{
  tests::Tester t(tg);
  ...
  for (unsigned nb = 0; nb < t.test_num(); ++nb)
	...
	if (t.verbose(tests::Tester::high))
	  std::cerr << "TEST: aut_to_exp_heuristic " << nb << " : test "
		    << w << std::endl;
	if (eval(a, w) ==
	    zero_as<semiring_elt_value_t>::of(a.structure().series().semiring()))
	{
	  break;
	  if (t.verbose(tests::Tester::high))
	    std::cerr << "TEST: aut_to_exp_heuristic " << nb
		      << " failed." << std::endl;
	}
      }
      if (i == nb_word_test)
  ...
  TEST(t, "aut_to_exp, first heuristic, basic test " + rate, success == t.test_num());
  return t.all_passed();
}

#endif // ! VCSN_TESTS_AUTOMATA_ALGOS_LABELED_GRAPHS_ELIMINATION_HEURISTIC_TEST_HH
